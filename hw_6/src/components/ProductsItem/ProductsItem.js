    ...
    <div>
      {isEditing ? (
        <>
          ...
          <button className="btn" onClick={toggleEditMode}>Отмена</button>
        </>
      ) : (
        <>
          <h3>Название товара: {name}</h3>
          <p> Описание: {description}</p>
          <p> Цена : {price}</p>
          <p> Доступность : {available ? "Доступен" : "Недоступен"}</p>
          <button className="btn" onClick={deleteItem}>Удалить</button>
          <button className="btn" onClick={toggleEditMode}>Изменить</button>
        </>
      )}
    </div>
  );
